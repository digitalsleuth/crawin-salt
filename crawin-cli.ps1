 <#
    .SYNOPSIS
        This script is used to install the Customize Robust Analysis Windows Forensic Environment (CRA-WIN) into a Windows VM or Windows machine
        https://github.com/digitalsleuth/crawin-salt
    .DESCRIPTION
        The Customized Robust Analysis Windows Forensic Environment (CRA-WIN) comes with a multitude of tools for use in conducting digital forensics using a Windows
    ...
    .PARAMETER Install
        Used to initiate the install of the CRA-WIN environment.
    ...
    .PARAMETER Mode
        There are two modes to choose from for the installation of the CRA-WIN Environment:
...
[string]$versionFile = "C:\crawin-version"
...
function Install-Saltstack {
    Start-Process -Wait -FilePath "C:\Windows\Temp\$saltstackFile" -ArgumentList '/S /master=localhost /minion-name=CRA-WIN' -PassThru | Out-Null
    if ($?) {
...
function Get-CRAWINRelease($installVersion) {
    $zipFolder = 'crawin-salt-' + $installVersion.Split("v")[-1]
    Write-Host "[-] Downloading and unpacking $installVersion" -ForegroundColor Yellow
    Start-BitsTransfer -Source https://github.com/digitalsleuth/crawin-salt/archive/refs/tags/$installVersion.zip -Destination C:\Windows\Temp -Dynamic
    Start-BitsTransfer -Source https://github.com/digitalsleuth/crawin-salt/releases/download/$installVersion/crawin-salt-$installVersion.zip.sha256 -Destination C:\Windows\Temp -Dynamic
    $releaseHash = (Get-Content C:\Windows\Temp\crawin-salt-$installVersion.zip.sha256).Split(" ")[0]
...
function Install-CRAWIN {
    $apiUri = "https://api.github.com/repos/digitalsleuth/crawin-salt/releases/latest"
    ...
    if ($Update) {
       if(-Not (Test-Path $versionFile)) {
           ...
           Write-Host "[!] CRA-WIN is not installed. Try running the installer again before choosing the update option." -ForegroundColor Red
           exit
        } else {
           $crawinVersion = (Get-Content $versionFile)
        }
        $installVersion = $crawinVersion
	} elseif ($Upgrade) {
        if(-Not (Test-Path $versionFile)) {
           $crawinVersion = 'not installed'
           Write-Host "[!] CRA-WIN is not installed. Try running the installer again before choosing the upgrade option." -ForegroundColor Red
           exit
       } else {
        $installVersion = $latestVersion
        }
    ...
    $errorLogFile = "C:\crawin-errors-$installVersion.log"
...
function Get-CRAWINVersion {
    if(-Not (Test-Path $versionFile)) {
        $crawinVersion = 'not installed'
    } else {
        $crawinVersion = (Get-Content $versionFile)
    }
    Write-Host "CRA-WIN is $crawinVersion" -ForegroundColor Green
    exit
}
