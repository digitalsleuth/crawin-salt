 <#
    ...
    .PARAMETER Mode
        There are two modes to choose from for the installation of the CRA-WIN VM:
...
[string]$versionFile = "C:\ProgramData\Salt Project\Salt\srv\salt\crawin-version"
...
function Get-CRAWINRelease($installVersion) {
    ...
    Start-BitsTransfer -Source https://github.com/digitalsleuth/crawin-salt/releases/download/$installVersion/crawin-salt-$installVersion.zip.sha256 -Destination C:\Windows\Temp
...
function Invoke-CRAWINInstaller {
    ...
    Write-Host "[-] Running CRA-WIN SaltStack installation" -ForegroundColor Yellow
    Install-crawin
}
...
Windows Forensics VM (CRA-WIN) Installer $installerVersion
Usage:
